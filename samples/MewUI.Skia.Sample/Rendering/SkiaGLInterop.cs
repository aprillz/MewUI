...
/// <summary>
/// Raw OpenGL entry points needed to allocate our own color texture + FBO for Skia's
/// <c>GRBackendRenderTarget</c>. Owning the GL texture handle lets us hand it back to
/// MewUI via <see cref="Aprillz.MewUI.Rendering.IExternalRasterSource"/> for zero-copy
/// sampling — Skia renders directly into it and MewVG samples from it within the same
/// GL context.
/// </summary>
/// <remarks>
/// Core GL 1.x calls (<c>glGenTextures</c>, <c>glBindTexture</c>, <c>glTexImage2D</c>) come
/// straight from <c>opengl32.dll</c> / <c>libGL.so.1</c>. FBO calls are 3.0+ so they're
/// resolved at runtime via the platform's <c>wglGetProcAddress</c> / <c>glXGetProcAddress</c>.
/// Mirrors the pattern in Skia.MewUI-main's <c>MewVGSkiaOpenGL</c>; isolated here so the
/// sample doesn't depend on any MewVG backend internals.
/// </remarks>
