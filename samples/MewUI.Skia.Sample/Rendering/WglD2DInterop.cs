...
/// context bootstrap so Skia GR GL has a current context to render into, plus
/// <c>WGL_NV_DX_interop</c> bindings to share a GL texture with a D3D11 texture, plus the
/// single ID3D11Device vtable slot we need to allocate that D3D11 texture against the D2D
/// backend's existing device.
/// </summary>
/// <remarks>
/// The pattern is "GL writes, D3D reads": Skia paints into a GL FBO whose color attachment
/// is registered with WGL_NV_DX_interop against a D3D11 texture. On unlock, the texture's
/// content is visible to D3D11 — D2D then samples it via the external raster source path.
/// </remarks>
internal static unsafe partial class WglD2DInterop
{
    // ---- Win32 user/gdi for hidden window + WGL context ---------------------------------
    ...
    // ---- WGL_NV_DX_interop function pointers (resolved at runtime) -----------------------
    ...
    /// <summary>
    /// Resolves the WGL_NV_DX_interop function pointers from the current GL context's
    /// driver. Must be called while a GL context is current (extension function addresses
    /// are per-context on Windows). Returns false if the extension isn't supported.
    /// </summary>
    ...
    // ---- D3D11 ID3D11Device::CreateTexture2D (vtable slot 5) -----------------------------
    ...
    public static int CreateTexture2D(nint d3d11Device, in D3D11_TEXTURE2D_DESC desc, out nint texture)
    {
        ...
        fixed (nint* pOut = &texture)
        {
            ...
            return fn(d3d11Device, pDesc, null /* initialData */, pOut);
        }
    }
