...
/// <summary>
/// macOS Metal GPU host for <see cref="Controls.SkiaCanvasView"/>. It renders Skia into a
/// backend-owned <see cref="IExternalWritableGpuSurface"/> and lets MewUI sample the same
/// surface without the sample allocating or exporting its own <c>MTLTexture</c>.
/// </summary>
/// <remarks>
/// The backend write scope exposes the <c>MTLTexture</c>, <c>MTLDevice</c>, and
/// <c>MTLCommandQueue</c> that are already valid for the surface. Skia only wraps those
/// handles for the duration of the surface lifetime; ownership stays with MewUI.
/// </remarks>
internal sealed class SkiaMetalSurfaceHost : IDisposable
{
    ...
    public bool EnsureSurface(int pixelWidth, int pixelHeight)
    {
        ...
        // Size changed → next Paint flushes sync to dodge the recreation-frame race.
    ...
    public IImage? Paint(Action<SKSurface> painter)
    {
        ...
        if (_renderSurface == null || _surface == null || _grContext == null)
        {
            return null;
        }
        ...
        // Sync only when the surface was just recreated (size changed since last Paint).
        // During a resize animation EnsureSurface fires every frame → sync every frame.
        // Steady-state frames hit the same-size early return in EnsureSurface, so this
        // flag stays false and we get full async GPU pipelining for free.
    ...
    private void EnsureGrContext(IExternalGpuWriteScope scope)
    {
        ...
        if (_grContext != null && _device == device && _commandQueue == commandQueue)
        {
            return;
        }
