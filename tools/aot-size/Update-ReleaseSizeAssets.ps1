...
[void]$tableSvg.AppendLine('<title id="title">MewUI publish size table</title>')
...
[void]$tableSvg.AppendLine("<text x=`"$tableLeft`" y=`"34`" font-size=`"24`" font-weight=`"600`">Measured sizes</text>")
...
for ($row = 0; $row -lt $entries.Count; $row++) {
    ...
    $values = @(
        ...
        "$(Format-Size $item.executableMiB) MB",
        "$(Format-Size $item.compressedMiB) MB"
    )
