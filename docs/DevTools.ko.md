...
**성능 모니터**는 최근 프레임 시간, 드로우 콜, 컬 비율을 보여주는 오버레이다. 프레임에서 가장 마지막에 그려지므로 자기 자신의 비용도 표시되는 수치에 들어간다. 드로우 콜은 그 프레임이 실제로 그린 양이다. 바뀐 작은 영역만 다시 칠한 프레임은 적게, 전체를 다시 칠한 프레임은 창 전체만큼 표시된다.
