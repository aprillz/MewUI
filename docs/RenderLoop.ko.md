...
프레임은 창 전체를 다시 칠하지 않는다. 이전 프레임 이후 무효화된 요소만 다시 그리고, 나머지 요소는 유지된 그리기를 재사용하며, 창에서 바뀐 영역만 다시 칠한다. 아무것도 바뀌지 않은 프레임에는 다시 칠할 것이 없다. 이것이 `OnRender`에 요구하는 사항은 [레이아웃](Layout.ko.md)의 Render 규칙에서 설명한다.
...
- 스케줄링은 프레임이 언제 실행되는지를 정하고, 무효화는 그 프레임이 무엇을 다시 칠하는지를 정한다. 연속 모드는 프레임을 연달아 실행하지만, 요소는 여전히 `InvalidateVisual()` 이후에만 다시 그려진다.
